...
- [대기] **S7-6: ★토폴로지 flip 결정 게이트 (GCP/결정, 비가역)** — 실제 group 증설 활성화(2nd generation). read fan-out no-regress 측정/논증. **완료 기준 = 결정 rendered**(trigger 없으면 DECLINE도 완료; dormant runway는 master에 잔존, flip만 보류). **add-protocol must-solve (S7-4서 inherit)**: ①add-window concurrent write LWW fence(plain-GET first-hit-stop이 writes-to-latest-gen 전역일관 전제 → fence 없으면 silent corruption) ②gen-0 capture/ordering(two-command: AddGen원본→PutShardGroup신규→AddGen확장). 전제: S7-2..S7-5 전부 완료.
