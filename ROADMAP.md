...
- [진행중] **S7-4: generation-probe read path (dormant)** — OpRouter가 FSM `PlacementGenerations()` 소비(write=latest gen, read=newest-gen-first probe), 빈 리스트→live candidate set fallback=byte-identical. **분해(advisor)**: 4a+4b 한 commit·code-gate / 4c 별도. **over-build 경계**: gen=1 byte-identical + 케이스별 2-gen happy-path 1개만. gen-0 capture는 S7-6 책임(4a는 FSM 리스트 agnostic 소비). 전제: S7-2 + S7-3.
  ...
  - **[대기] S7-4c** version-aware: ①ListObjectVersions 전 gen union+merge ②GetObjectVersion/HeadObjectVersion 옛 gen probe ③조건부 PUT If-Match/If-None-Match cross-gen CAS + ReadAt/ACL/Tags probe 전환. 셋은 first-hit-stop 불가.
