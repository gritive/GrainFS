...
- [대기] **S4-4c: GET/HEAD read-path quorum-meta migration** — S4-4b write-path 제거로 노출됨: GET/HEAD/ReadAt/Truncate read 경로(`cluster_coordinator.go` `routeIndexedReadOrBucket` + `objectMatchesIndexForFollowerRead` 5 call-site + `op_routing.RouteObjectRead`)가 제거된 object index 대신 **quorum meta read**(scatter-gather/quorum)로 객체 메타데이터를 읽도록 재작성. 현재 follower-local-read가 빈 인덱스와 비교→항상 forward→internal bucket서 "router not configured". 11개 unit + 1개 ginkgo(`ec_fix_integration_test.go` internal-bucket Truncate)가 `t.Skip("Phase 4 S4-4c: ...")`로 대기 중 — 이 슬라이스서 새 계약으로 갱신. `VolumeReplicaSummaries`도 여기서 quorum-meta 기반 복원. 전제: S4-4b.
