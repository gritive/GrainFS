...
- [보류] **S5-2: ★결정 벤치 실행 (merge go/no-go) — BLOCKED by a real multi-host raft bug (NOT a devel NO-GO; verdict UNMEASURED)** — `benchmarks/gcp/bench_gcp_cluster.sh`를 실제 GCP 4-node(n2-standard-4, asia-northeast3) + warp client로 실행(2026-06-11). **근본 원인(investigate로 정밀 격리, healthy 4노드 클러스터에서 재현)**: 비-leader 노드로 들어온 PUT/GET/HEAD는 그룹 leader로 forward되고, leader는 `forward_receiver.go`의 `waitForwardReadFence → gb.ReadIndex(forwardReadFenceTimeout=5s)` linearizable-read 배리어를 통과해야 한다. **실제 멀티호스트에서 이 ReadIndex(group-raft heartbeat-round majority 확인)가 5s 내 완료되지 못하고 stuck → `context deadline exceeded` → forward `internal reply error` → PUT의 previous-object-lookup(HeadObject) 실패 → PUT/GET/HEAD 전멸**(conc1/64KiB 단일 요청도 53s 실패 = load/pool-starvation 아님). 펜스를 쓰지 않는 `handleListObjects` forward는 **성공**(node-1 로그 status OK)=ReadIndex 펜스가 정확한 culprit. **2차 증상**: joiner 부트 시 `WaitDEKReady`(60s) 타임아웃 사망(meta-raft AE catch-up 미완)=같은 근원(raft-over-mux의 heartbeat/AE reply round가 실제 멀티호스트에서 신뢰성 있게 완료 안 됨)으로 추정. **devel·master 동일**(같은 raft 코드·타임아웃 상수)=devel 회귀 아님. 기존 테스트가 못 잡은 이유: 멀티노드 테스트는 fake-transport(in-proc) 또는 single-host-multi-port(local bench, RTT≈0이라 ReadIndex 즉시 완료)만 사용 — **실제 멀티호스트 raft-over-mux는 한 번도 실측 안 됨.** 배제됨: Nagle(SetNoDelay 적용), per-message TLS handshake(mux 스트림 풀 영속), pool-starvation(conc1도 실패), peer-address 해석(정상), 80ms 단순 bump(클러스터는 leadership 유지=heartbeat은 작동, ReadIndex round-confirm만 실패). **fix 시도 1 (REFUTED, reverted)**: mux carrier establishment(muxPoolSize TLS handshake)를 tight per-RPC ctx(80ms)에서 분리(`muxEstablishTimeout=10s`). unit-correct(raft 테스트 통과)이나 멀티호스트 실측서 forward 실패 동일 지속 → establishment가 sole 근본원인 아님 → reset(미커밋). **계측 시도도 함정**: raft actor hot-path(`handleHeartbeatReply`/`tryFlushReadIndex`)에 동기 `fmt.Fprintf(stderr)`를 넣자 단일-goroutine actor가 stderr I/O에 블록→meta-raft 스톨→**genesis가 gen-0 DEK Apply 지연으로 WaitDEKReady 타임아웃 사망**(Heisenbug — 계측이 타이밍 교란). **정밀 격리(비-perturbing 에러-wrap 진단, REVERTED 후)**: forward 실패는 `waitForwardReadFence`의 **`gb.ReadIndex`가 아니라 `gb.WaitApplied`** 단계서 타임아웃(전 노드 에러 100% "readfence WaitApplied", ReadIndex/quorum-read는 0건). 즉 **ReadIndex는 성공(barrier idx 반환)하나 `raftNodeAdapter.WaitApplied`(raftnode_adapter.go:168)의 `a.n.CommittedIndex()>=barrier` 폴링이 5s 내 충족 안 됨** = group FSM commit-snapshot(`n.rs.Load().commitIndex`)이 ReadIndex barrier(`n.st.commitIndex` live)에 미달. ReadIndex round-confirm 가설은 **틀림**(ReadIndex 성공). 유력 메커니즘: forward가 stale leader-hint로 non-leader 도달→gb.ReadIndex가 실제 leader로 forward해 leader commitIndex barrier 획득→로컬(follower) commit이 catch-up 못 해 WaitApplied 타임아웃 (또는 leader 자체 snapshot publish lag). **2차 발견**: genesis 노드도 멀티호스트서 `WaitDEKReady` 간헐~연속 실패(계측 없는 run에서도)—gen-0 DEK를 직접 생성하는 genesis조차 60s 내 keeper.HasActiveGen 미달=apply/commit/DEK 머신리가 멀티호스트서 광범위 불안정. **막힘**: 정확한 barrier-vs-committed-vs-isLeader 값 캡처가 worn SPOT 4-vCPU 클러스터의 genesis 부팅 불안정으로 미완(여러 사이클서 genesis가 안 뜸). **남은 작업**: 안정 repro 환경(non-SPOT/큰 VM 또는 로컬 netem 멀티호스트, 4-vCPU 리소스 경합 변수 제거) + raftNodeAdapter.WaitApplied에 barrier/committed/isLeader 비-perturbing 로깅(read-path 폴링 루프, actor 밖이라 안전)으로 leader-hint-stale vs snapshot-publish-lag 확정 → 타겟 수정. 전제: S5-1 + 이 버그 fix. **fix 시도2(WaitApplied 격리용 진단)는 REVERTED**(temp). 체계적 멀티호스트 apply/commit 불안정이라 단일 타겟 수정 전 안정 repro 확보가 선결.
