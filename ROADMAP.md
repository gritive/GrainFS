...
- [대기] **S5-2b: ★결정 벤치 실행 (merge go/no-go)** — forward가 고쳐졌으니 이제 cross-binary A/B(devel-fixed vs master) 실행 가능. **단 master(옛 전체)도 같은 read-fence 코드를 가지면 동일 데드락 → master arm이 숫자를 못 냄**: A/B 전에 master의 멀티호스트 forward 동작 확인 필요(master read-path가 Phase 1-4 이전이라 다른지, 아니면 fix를 master에도 cherry-pick할지 결정). verdict 규칙: ① PUT win AND ② GET/HEAD no-regress → GO면 Phase 6 후 merge, NO-GO면 브랜치 폐기. 전제: S5-2a.
