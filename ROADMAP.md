...
- [대기] **S7-4: generation-probe read path (dormant)** — read-miss 시 직전 generation fallback probe(bounded fan-out). single-generation일 때 probe 분기 미진입(byte-identical). **version-aware probe(필수)**: 단순 GET=newest-gen-first first-hit-stop, 그러나 ①versioned bucket ListObjectVersions=전 gen union+merge ②GetObjectVersion(특정 versionID)=옛 gen까지 probe ③조건부 PUT If-Match/If-None-Match=gen 횡단 최신 확정 후 CAS — 셋은 first-hit-stop 불가. 검증: 2-generation in-proc 옛-gen 객체 read 성공+세 케이스, gen=1 byte-identical RED-on-revert. 전제: S7-2 + S7-3.
