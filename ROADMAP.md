...
- [보류] **S5-2: ★결정 벤치 실행 (merge go/no-go) — BLOCKED by a real multi-host cluster bug (NOT a devel NO-GO)** — `benchmarks/gcp/bench_gcp_cluster.sh`를 실제 GCP 4-node(n2-standard-4, asia-northeast3) + warp client로 실행함(2026-06-11). genesis 노드는 정상 부트·서빙(PUT status 200, ~1.1s/10MiB). **그러나 joiner 노드(1..N-1)가 부트 중 `Error: DEK readiness: WaitDEKReady: context deadline exceeded`(dekReadyBootTimeout=60s, `internal/serveruntime/dek_readiness.go`)로 사망** → 클러스터가 genesis 단독으로 붕괴 → 비-genesis 노드로의 forward가 `forward: no reachable peer` / `context deadline exceeded`로 실패 → warp가 전 op 실패(no publishable benchdata). 근본 원인 = 실제 멀티호스트 네트워크에서 gen-0 DEK의 meta-raft 전달/catch-up이 60s 내 완료되지 못함(클러스터가 16 shard-group 시딩에 ~7.5분 소요 = meta-raft 복제가 병적으로 느림). **`--raft-heartbeat-interval 1s --raft-election-timeout 3s`로 raft 부하를 낮춰도 동일 실패** → heartbeat storm이 아닌 더 깊은 inter-node 복제 문제. **devel·master 두 바이너리 모두 동일하게 실패** = devel 회귀가 아님 → **Phase 5 verdict = UNMEASURED (NOT NO-GO).** 기존 테스트가 못 잡은 이유: 멀티노드 테스트는 fake-transport(in-proc) 또는 single-host-multi-port(local bench)만 사용 — **실제 멀티호스트 forward/DEK 전달은 한 번도 실측된 적 없음.** 다음 액션: 이 멀티호스트 부트 버그를 전용 `investigate`로 근본 수정해야 S5-2 verdict 산출 가능(전제: S5-1, 그리고 이 버그 fix).
