...
| RustFS   | Rust, 소형 객체 빠름           | Go 생태계, 로컬→Cluster 확장, Volume Device       |
| Ceph     | 검증됨, Object+Block+File      | 싱글 바이너리, 운영 경량                          |

**GrainFS 고유 포지션:** QUIC 전송 + Erasure Coding + Volume Device (NBD/NFS) + 로컬→Cluster 확장. 전부 싱글 바이너리.

## 4. 단계별 로드맵

### Phase 1: 단일-노드 S3 API ✅
...
### Phase 3: 로컬 → Cluster 전환 + 분산 스토리지 ✅

**목표:** 단일-노드 인스턴스를 무중단으로 클러스터 노드로 전환한다.

- Storage Backend 인터페이스 정의 (로컬 ↔ 분산 교체 가능) ✅
- 단일-노드 인스턴스 → 클러스터 seed 노드 전환 워크플로우 ✅
...
**검증:**
- 단일-노드 → 3노드 전환 시 데이터 무손실, 서비스 중단 없음
...
- **읽기 캐시**: ✅ CachedBackend (LRU, 바이트 크기 기반 관리). 단일-노드 모드에서 GetObject/HeadObject 캐시, PutObject/DeleteObject/CompleteMultipartUpload 시 자동 무효화. 클러스터 모드는 데이터 일관성을 위해 캐시 미적용 (Phase 9에서 Raft FSM 기반 무효화 검토)
...
- **Raft 로그 GC 상시 활성** ✅ — `--badger-managed-mode` 플래그 제거 (v0.0.171.0). 관리 모드가 유일한 레이아웃; 비관리 모드로의 복귀 불가.
