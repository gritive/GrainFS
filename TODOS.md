...
- [ ] **PeerHealth one-strike-out** — *정합성 위협 (silent fallback)* — `internal/cluster/peer_health.go` 의 `MarkUnhealthy` 가 단 1회 EOF 로 호출되면 cooldown 10초 동안 `IsHealthy=false`. 그동안 `putObjectNxSpooled` 의 peer loop (line 867-870) 가 해당 peer 를 skip — debug-level 로그만 남김. cooldown 만료되면 `IsHealthy` 자동 true 복귀. **위험:** 운영 시 silent skip 이 N×replication 보장을 약화시킴 — peer 가 일시적으로 unstable 일 때 단일-replica 로 fall back 하면서도 운영자에게 안 보임. **개선 방향:** (a) skip 시 warning 레벨 로그 + counter 노출, (b) MarkUnhealthy 임계값 (N consecutive failures) 도입, (c) cluster status admin endpoint 에 peer-health 노출, (d) `tests/e2e/volume_scrub_multinode_test.go` 의 12s cooldown 우회 sleep 제거 가능.

- [ ] **Silent N×replication skip — debug 레벨만** — *모니터링 gap* — `putObjectNxSpooled` line 868: `b.logger.Debug().Str("peer", peer).Msg("skipping unhealthy peer for replication")`. info 레벨 운영에서는 안 보임. 첫 실패만 warn ("data replication failed"), 그 후 모든 skip 이 silent. **개선:** (a) warning + counter (e.g. `grainfs_replication_skipped_total{peer,bucket}` Prometheus metric), (b) admin/health endpoint 에 "writes succeeded with <N replicas, expected M" 등 표면화, (c) 운영자 알림용 threshold (예: 3-node 클러스터에서 1 replica 로 fall back 시 critical).
