...
- [ ] **QUIC→TCP migration — S1 TCPTransport landed dormant; wiring-slice (S2/S3) must-fix items**
   ...
   - [ ] **[S5c-2 — §6 parity bench, USER-RUN, multi-node Linux infra] DECIDES the flip.** Run the §6 mixed-concurrency workload on a real multi-node TCP cluster (`--transport tcp`) vs QUIC. **This bench IS the multi-node serveruntime validation** (cluster formation + invite-join + S3 over TCP) that S5c-1 deferred. TCP must clear ≥1.0x throughput + CPU/GB-at-TCP-level + control-plane p99 (§6 decision rule); if not, QUIC stays and the flip does NOT happen. The agent cannot run this (no multi-node Linux); the user provisions infra (as on the prior GCP run).
   - [ ] **[S5c-3 — the flip, GATED on S5c-2 ≥1.0x] + transport-persistence hardening.** Change the default → TCP + CHANGELOG. **[from S5c-1 code gate, finder 2 #1]** PERSIST the cluster's transport selection (node-wide, like the cluster key) so a node boots the right transport across restarts regardless of `--transport` flags — until then `--transport` MUST be passed consistently across restarts (bootClusterTransport AND both invite-join phases all re-derive the transport from the flag; a restart that drops `--transport tcp` boots a fully-QUIC node that can't rejoin a TCP cluster). This belongs to the flip (TCP becomes the durable default), not S5c-1.
