...
- **[P1][S4c-d HARD precondition] Bucket-name reuse after deleting a `soleauth=on` bucket.** `applyDeleteBucket` deletes only `BucketKey` (apply.go:299), NOT `soleauth:{b}` / `soleauthepoch:{b}` / `bucketver:{b}` / leftover `obj:`/`lat:` records; `applyCreateBucket` recreates only `BucketKey`. Under `on`, `DeleteBucket`/force-delete intentionally leave non-carve-out vid-bearing FSM records as orphans (non-authoritative, GC'd by the orphan scrubber), and the terminal `on` state + epoch persist. So a recreated same-name bucket would inherit a stale `on` state + epoch + orphan records. DORMANT today (no bucket reaches `on` until S4c-d, so there is no `on` bucket to delete-then-recreate). When S4c-d makes `on` reachable it MUST clear per-bucket soleauth/epoch/versioning state + force orphan cleanup on bucket delete before the name can be reused (or make `applyDeleteBucket` clear those keys). Surfaced by the S4c-c-read2b plan-gate (codex R2).
- **[P1][S4c-d HARD precondition] read1 single-object `on` reads must be decode-strict (fail-closed) before flip.** read1's `on` branches (object_get.go:215 HEAD/GET-latest, object_version.go:58 specific-version, bucket.go:524 GetObjectTags) read via the TOLERANT `readQuorumMetaVersions`/`readQuorumMetaVersion`, which silently drop a per-blob decode failure (and `readQuorumMetaVersionsStrict` is peer-availability-strict but NOT per-blob-decode-strict — its comment at per_version_cutover_verify.go:48-54). Under sole authority a corrupt/undecodable delete-marker-latest blob would be omitted → `deriveLatestVersion` sees only an older live version → HEAD/GET **resurrects a deleted object**. DORMANT today (no bucket `on`, so unreachable). Surfaced by the S4c-c-read2a code-gate (codex). **Fix (a dedicated read1-hardening follow-up slice, before S4c-d makes `on` reachable):** add a decode-strict per-key local reader + per-key RPC path (propagate per-blob decode errors, mirroring the read2a all-version client fix) and route read1's three `on` single-object paths through it. User-approved deferral (dormant + substantial, out of read2a's ListObjectVersions scope).
