...
- [ ] SIMD
- [ ] **Predictive resource warnings — BadgerDB vlog (PR2)** — *zero ops* — Goroutine watcher 는 v0.0.44.0 (PR1) 에서 land. `internal/resourcewatch/goroutine_provider.go` + `cmd/grainfs/resource_monitors.go` 의 `startGoroutineResourceMonitor`. 측정 기반 default — 5000 warn / 20000 critical (3-node cluster idle baseline ~200, 25× / 100× headroom). **남은 작업:** BadgerDB value log 크기 추세 추적. 디스크 walk 로 `*.vlog` 합산, path-prefix 기반 categories (raft / state / incident / receipt / eventstore). 별도 design (`docs/superpowers/specs/2026-05-05-predictive-resource-warnings-design.md` 의 PR2 섹션). Re-open trigger: vlog default 임계값을 결정할 production 측정 (단일 노드 누적 vlog bytes p95).
...
- [ ] **EC scrub `BlockSource` adapter — CLI/admin surface land** *(deferred — adapter shipped internally v0.0.43.9, surface blocked on multi-raft fan-out)* — `internal/scrubber/ec_source.go` 에 `ECScrubSource`/`ECScrubVerifier` 구현 + 단위 테스트 11개 통과. `BlockSource.Iter` 시그니처에 `bucket` param 도 v0.0.43.9 에서 추가 (Director.runSession 이 sess.bucket 전달). 그러나 **CLI/admin surface 는 ship 안 함** — `*cluster.DistributedBackend.ScanObjects/ListBuckets/HeadBucket` 이 group-0 의 BadgerDB 만 walk 하므로 SeedGroups>=1 + 동적 group 할당 환경에서 Checked=0 으로 떨어짐 (실측 — SeedGroups=1 에서도 모든 노드 Checked=0). **Re-open trigger:** (a) Multi-raft fan-out 설계 — `Scrubbable` 을 모든 owned `*GroupBackend` 위에 union 하는 wrapper 또는 `clusterRouter` 가 bucket→group 라우팅을 source 에 노출, (b) 위 (a) 후 `serve.go` 에 `director.Register("ec", ecSource, ecVerifier)` + admin endpoint `POST /v1/scrub` + CLI `grainfs scrub <bucket>` 추가, (c) e2e 가 실제 Detected≥1 + Repaired≥1 검증. design doc: `docs/superpowers/specs/2026-05-05-ec-scrub-blocksource-design.md` (D1-D9 결정 보존).
