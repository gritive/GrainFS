...
- [ ] **E2E test cold-start race — peer-warmup helper** — *test infra* — `tests/e2e/volume_scrub_multinode_test.go` 와 `tests/e2e/nbd_multinode_replication_test.go` 가 cluster 시작 후 12s `time.Sleep` 으로 peer connection 이 warm 해지길 기다림. 이건 PeerHealth threshold 결정과는 별개 concern — peer connection 이 cold 한 상태에서 첫 inter-node write 가 race 하기 때문. 개선: `startE2ECluster` 또는 별도 helper `waitForPeersHealthy(c, timeout)` 가 admin `/v1/cluster/peers` endpoint 를 polling 하거나 노드 간 explicit cross-ping 으로 모든 peer 가 reachable 함을 확인 후 return. 이러면 (a) 12s 고정 sleep 이 actual readiness signal 로 대체, (b) test 시간 단축 (warm 빠르면 즉시 진행), (c) PeerHealth threshold 변경과 decoupled.
