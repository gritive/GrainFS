...
- **[P2][roadmap] S4c-b wires the all-versions enumerator** — the snapshot restore-mode absent-blob purge (`snapshotable.go`) must enumerate the on-disk per-version blob tree via `ScanQuorumMetaVersionsBucketAll` (local) + fan out to peers via `ScanQuorumMetaVersionsAll` (FAIL-CLOSED: a partial enumeration could leave a newer absent blob that wins max-VID derive), and force-delete any `(bucket,key,vid)` absent from the snapshot. The cluster-wide fan-in/merge lives with that consumer (a3 shipped only the local scan + per-peer RPC).
