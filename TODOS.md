...
- [ ] **incident store scope index** — `internal/incident/badgerstore/store.go` 의 `List(ctx, limit)` 외에 `ListByScope(ctx, kind, id, limit)` 추가. Phase B 의 `volume scrub status`는 일단 `List(500) + 메모리 필터`로 동작. 운영에서 incident 누적이 수만건+ 가 되어 status 응답이 100ms 초과하면 secondary index(`scope:<kind>:<id>:<-ts>:<id>`) 도입. **Trigger:** `volume scrub status` p95 latency > 100ms 또는 incident store size > 50k entries.
- [ ] **`ScanObjects(bucket, keyPrefix)` 로 시그니처 확장** — `internal/cluster/scrubbable.go` 의 `ScanObjects` 가 `lat:` 인덱스 전체를 iterate. `Director.Trigger(bucket, keyPrefix)` 는 결과를 prefix 필터하지만 iterator 자체는 전체 순회. 볼륨 다수 + 인덱스 GB 단위 시 비용 증가. Badger iterator 의 SeekPrefix 활용해 prefix-bounded scan 추가. **Trigger:** `volume scrub` p95 latency > 5s 또는 lat 인덱스 size > 500MB.
