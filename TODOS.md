...
- **[P1][NEW S4c-d HARD precondition] Multi-group soleauth epoch source + per-group readiness.** The boot-window fix above is correct+complete for SINGLE-group (group-0) clusters only. The soleauth epoch fence uses a SINGLE callback on the shared `ShardService`; in a multi-group node every owned `GroupBackend` re-wires it (`group_backend.go:125` → the LAST group-install wins, binding it to that group's `GetBucketSoleAuthEpoch`), each group has its own apply loop (`boot_phases_storage_runtime.go:490`), and groups are added at RUNTIME (514) — so the fence reads the WRONG group's epoch for buckets owned by other groups, and a single global `soleAuthEpochReady` cannot track per-group catch-up. PRE-EXISTING (a2-A) + DORMANT. Before S4c-d flips a bucket in a multi-group cluster, the epoch source must be **bucket-routed to the owning group's backend** AND readiness must be **per group** (a group is reliable only after ITS apply loop applies its boot-committed backlog, re-gated on each runtime install). The boot-window slice is strictly-safer-not-worse for multi-group (fail-closed during boot vs today's fail-open). Surfaced by the boot-window code-gate (codex R2).
