...
- [ ] **R+H Coalescer reply race fix (PR-A, 선행)** — `/plan-eng-review` + codex 발견 (P1 #5): `HeartbeatCoalescer.flush()`가 `inFlight.Store` 전에 `SendHeartbeatBatch` 호출. 빠른 receiver 응답이 corrID 미등록 상태에 도착하면 드롭 → group raft 200ms/1s timing엔 묻혀있지만 e2e flake 일부 원인 가능, meta-mux(750ms election) 끼우면 폭발. 수정: corrID reservation BEFORE send. 회귀 테스트 `TestCoalescer_FastReplyBeforeRegister`. ~80 LOC. design doc §Follow-up 참고.
- [ ] **Meta-raft mux 통합 (PR-B, PR-A 후)** — R+H (v0.0.16~v0.0.17)에서 per-group raft만 mux 적용. meta는 ~4% 트래픽. **wire-compat 결정**: magic groupID `__meta__` 사용(1-byte discriminator 불필요). codex 7-finding 반영 plan v1: dispatchToLocalGroup 분기 + `__meta__` reservation 4곳 + startup 순서 재배치 + fallback budget split + mixed-version error fallback. ~380 LOC. 사전조건: PR-A + R+H load-N8 clean 측정. design doc §Follow-up: Meta-raft mux integration 참고.
