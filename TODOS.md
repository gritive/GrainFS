...
- [ ] **QUIC→TCP migration — S1 TCPTransport landed dormant; wiring-slice (S2/S3) must-fix items**
   ...
   - [ ] **[P3→S4] TCP mux has no capability-exchange equivalent.** QUIC's `GetOrConnectMux`/inbound path run `doCapabilityExchangeDial`/`handleCapabilityExchange` (version/feature negotiation) before handing the conn to the mux handler. The TCP carrier delivers raw `net.Conn`s to RaftConn and carries version only in the ALPN (`tcpMuxALPN`). At the S4 wiring slice, confirm no OTHER negotiated capability is load-bearing (if one is, add a TCP equivalent before flipping boot).
   ...
   - [ ] **[S4 acceptance criterion] Prove entries-AE lands on a bulk stream under a REAL split.** S2c-1's call-site routing (entries-AE→`CallBulk`) is inspection-verified only (2 finders); on single-lane QUIC `CallBulk`==`Call` so no test distinguishes them. A functional TCP cluster test would ALSO pass if entries-AE were mis-wired to `Call` — it would just HoL-block under load, invisible to pass/fail (the `dev벤치≠패리티` trap). **So S4 (TCP wiring) MUST include an explicit assertion** that with `SetMuxBulkLaneStreams(k>0)`, an entries-bearing AppendEntries is written on a bulk-lane stream (and a concurrent heartbeat on a control-lane stream), not merely "the cluster works."
