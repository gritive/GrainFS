...
- **[P2][roadmap] S4c-a2 — arm the fence.** Wire the `soleauth` tri-state into the four quorum-meta leaves: per-bucket fence `RWMutex` (no-op when `off`), the additive `admitted_soleauth_epoch` scalar on the write/delete shard envelopes (epoch 0 = stale/un-upgraded peer ⇒ reject during cutover), a `ShardService` bucket-state read callback, and the S3-edge epoch stamp. The flag shipped in S4c-a1 is INERT (read by nothing in the read/write/fence paths) precisely so this wiring lands as its own reviewable slice.
