...
- [ ] **Volume scrub — dedup 모드 호환** — *zero ops 무결성 oracle (dedup 활성)* — v0.0.43 의 `ReplicationObjectSource` 는 `__vol/<vol>/blk_<NNN>` 키 패턴 가정. 그런데 `--dedup=true` (기본값) 일 때 volume Manager 가 블록을 content-addressed 키 `blk_<NNN>_v<UUID>` 로 저장 (`internal/volume/volume.go:608`). 그래서 이번 PR 의 e2e 는 모두 `--dedup=false` 로 구동. **수정 방향:** (a) source 가 dedup 인덱스를 통해 canonical 키 reverse-lookup, 또는 (b) Walk 가 emit 한 모든 `_v<UUID>` 객체를 verify (다른 volume 의 동일 canonical 도 같은 ETag oracle 적용 가능), 또는 (c) volume Manager 가 dedup 모드에서도 `blk_<NNN>` "logical" 키를 유지하고 dedup 은 별도 sidecar 인덱스. v0.0.44 의 dedup-aware scrub design.

- [ ] **Volume scrub — 다중 노드 repair E2E** — v0.0.43 의 e2e 는 단일 노드 (1-node cluster) 만 커버. RepairReplica 의 peer-pull 경로는 단위 테스트도 통합 테스트도 없음 (위 항목 `RepairReplica 단위 테스트` 와 함께). 후속에서 `tryStartStaticMRCluster` 기반 3-node E2E 추가 — 한 노드의 블록을 truncate, scrub trigger, peer 에서 healthy bytes 가 가져와져 local 복구되는지 확인. `--detach` 후 `volume scrub status` polling.
