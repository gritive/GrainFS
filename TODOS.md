...
- [ ] **QUIC→TCP migration — S1 TCPTransport landed dormant; wiring-slice (S2/S3) must-fix items**
   ...
   - [ ] **[S5 — boot flip + bench gate, SIGN-OFF REQUIRED] remaining before TCP goes live:** (b) boot transport-selection plumbing — construct the TCP stack (base+mux+meta+join) behind a config flag (default QUIC) + a functional TCP-cluster boot/elect/write test (dormant, no sign-off); (c) flip the default → TCP + the §6 multi-node parity bench + the [from S4] Join RST-robustness netem/multi-node test above (TCP-live, **needs explicit sign-off + multi-node Linux infra**).
