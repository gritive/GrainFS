...
- [ ] **raft-ehn Tier 3** (별도 브랜치, 각 sub-project는 독립 design + plan + worktree):
  - **Tier 3-1: Joint consensus + Learner** — Multi-Raft atomic move-replica · multi-server replacement 안전성. wire format `JointConfChange=2` + `new_config:[string]` / `old_config:[string]` 슬롯 reserved (PR-A에서). 세 sub-project로 분해:
    ...
    - **Sub-project 2: Joint consensus core** — §4.3 wire activation, 4-state machine (normal → C_old+new → C_new), dual-quorum, ConfChange entry encoding. ~5 PR / ~1500 LOC. **진행 중** (worktree `feat-raft-joint-consensus-core`, design 2026-04-30). plan-eng-review 통과: PR-J5/J6 합쳐 5 PR로 축소, leader change wakeup은 모든 노드 apply path의 close hook으로 자동 처리 (재설계 불필요), AddOnly/RemoveOnly/no-op edge case 테스트 추가.
