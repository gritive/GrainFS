...
- [ ] **raft-ehn Tier 3** (별도 브랜치, 각 sub-project는 독립 design + plan + worktree):
  - **Tier 3-1: Joint consensus + Learner** — Multi-Raft atomic move-replica · multi-server replacement 안전성. wire format `JointConfChange=2` + `new_config:[string]` / `old_config:[string]` 슬롯 reserved (PR-A에서). 세 sub-project로 분해:
    ...
    - **Sub-project 2: Joint consensus core** — §4.3 wire activation, 4-state machine (normal → C_old+new → C_new), dual-quorum, ConfChange entry encoding. **완료** (v0.0.6.16~v0.0.6.20, PR #108~#112). plan-eng-review 통과 + 5 PR 시퀀스: J1 FBS schema, J2 dual-quorum, J3 apply/auto-progression/truncation revert, J4 caller API + commit-time close, J5 snapshot persistence + self-removal step-down.
    - **Sub-project 2 후속: PR-A reserved field cleanup** — `ConfChangeEntry.new_config:[string]`/`old_config:[string]` 두 deprecated field 제거. Sub-project 2 PR-J1에서 FBS `(deprecated)` attribute 표시. mixed-version 호환 윈도우 종료 후 (다음 minor version) 별도 cleanup PR. 잊혀짐 방지로 본 entry 유지.
    - **Sub-project 2 후속: Joint phase observation API** — process restart 시 caller (proposeJointConfChangeWait 호출자) lost 위험 (cross-model 발견 Codex #1). Sub-project 3 ChangeMembership 공개 시 idempotent reattach + `Configuration()`/`JointPhase()` query API 노출 검토. 본 sub-project는 in-process channel 한정.
    - **Sub-project 2 후속: Stuck joint abort 메커니즘** — JointEnter committed 후 C_new가 절대 majority 도달 못하는 시나리오 (cross-model 발견 Gemini #7). 현재는 jointPhase != JointNone 가드로 모든 propose 차단 → 클러스터 deadlock. Force-abort entry 또는 운영 도구로 jointPhase reset. Sub-project 3 또는 별도 후속 PR.
    - **Sub-project 2 후속: Voter set lock-free read** — `n.mu` hold 안에 voter set read가 끼어 있어 read-side 락 시간 길어짐. `atomic.Pointer[voterSets]` (jointPhase + jointOldVoters + jointNewVoters + config.Peers를 immutable snapshot으로 묶어 COW swap)로 빼면 hasQuorum/quorumMinMatchIndex 등 read 경로 lock-free화 가능. 단 raft.go 전반의 multi-field invariant (term + state + log + commitIndex + matchIndex) 안에서 voter set만 분리하려면 여러 개의 atomic.Pointer COW set 도입 필요 — 본 sub-project 5개 PR 모두 머지 후 별도 brainstorming + design.
    - **Sub-project 2 후속: Joint chaos scenarios + E2E snapshot mid-joint** — PR-J5에서 internal-only `proposeJointConfChangeWait`이라 chaos package(별 package)에서 호출 불가하여 skip. Sub-project 3 `ChangeMembership` public API 노출 후 chaos scenarios 3개 (LeaderCrashBetweenEnterAndLeave / PartitionDuringJoint / RepeatedLeaderChange) + `TestJoint_E2E_SnapshotMidJoint_AutoCompletes` 작성.
