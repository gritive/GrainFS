...
- [ ] **QUIC→TCP migration — S1 TCPTransport landed dormant; wiring-slice (S2/S3) must-fix items**
   ...
   - [ ] **S2c (NEXT) = lane separation + per-conn failure isolation** via driver composition; reconcile the HB coalescer's conn-level `NextHeartbeatCorrID` with the per-lane split.
