...
- [ ] **QUIC→TCP migration — S1 TCPTransport landed dormant; wiring-slice (S2/S3) must-fix items**
   ...
   - [ ] **S2b-2 (NEXT) = TCP mux driver, dormant.** A TCP `MuxCarrier`: `OpenStream` = dial a peer conn, `AcceptStream` = yield the next grouped inbound conn, inbound demux that groups N TCP conns into one peer carrier (the N-conn-per-peer model), + `GetOrConnectMux`/`EvictMux`/`SetMuxConnHandler` on `TCPTransport` (closeHook = `net.Conn.Close`). This is where the race-loser hazard below is reworked (each RaftConn owns its conns outright → the shared-conn dedup likely dissolves) and where the net.Pipe / TCP-loopback carrier proves the S2b-1 abstraction on a non-QUIC carrier.
   - **S2c** = lane separation + per-conn failure isolation via driver composition; reconcile the HB coalescer's conn-level `NextHeartbeatCorrID` with the per-lane split.
