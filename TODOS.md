...
- **[DONE / ELIMINATED] Routed-cluster per-node snapshot for soleauth-on buckets — REMOVED by deleting the object-metadata snapshot feature.** Rather than build the per-node snapshot mini-epic to make soleauth-on buckets cluster-snapshottable, the entire object-metadata snapshot feature (auto-snapshotter + `/admin/snapshots` API + capture/restore + `snapshot.Manager`) was **removed** (user decision: "snapshot 전면 제거"). With no object snapshot at all, there is no routed snapshot of `on` buckets to make work, so this precondition is moot. **KEPT** the live-version manifest the scrubber needs (`DistributedBackend.ListAllObjectsStrict` + the `storage.SnapshotObject` manifest type, relocated to `object_manifest.go`) and the Raft snapshot (log compaction, a different feature). The EC orphan-shard/segment GC frozen sources are wired to explicit empty no-ops (NOT nil — nil fails closed and disables the sweep). BREAKING: `/admin/snapshots` and the `snapshot-interval`/`snapshot-retain` cluster-config keys are gone. ~5000 LOC removed.
...
- **[P1][S4c-d HARD precondition] Gate the `soleauth=on` activation path (the admin flip) behind verifier-clean + read-fence + boot-window.** The direct snapshot-restore activation path is **gone** (the object-metadata snapshot feature was removed), so the admin flip is now the SOLE path that can first make a bucket `on`. Today no path produces a *first* `on` (the admin flip is unbuilt), so no `on` bucket can exist and the S4c-c-read1 `on` read branch is unreachable. When S4c-d builds the admin flip, it must enforce: (1) verifier `gaps+stuck+unknown+verify_errors==0` — note MPU completion mirrors the per-version blob **best-effort/warning-only** (object_put.go:414), so a missing MPU blob shows as gap/stuck and the flip must refuse; (2) the forwarded-read fence below; (3) the boot-window fence (above). Surfaced by the S4c-c-read1 plan-gate (codex Q2/Q4).
