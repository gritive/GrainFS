...
- [ ] **QUIC→TCP migration — S1 TCPTransport landed dormant; wiring-slice (S2/S3) must-fix items**
   ...
   - [ ] **S5 acceptance criteria (REQUIRED — the boot flip is NOT just "flip the flag + run the bench").** Three things were deferred to S5 across S2b-2/S2c-1/S4, each invisible to a green functional/bench run (the `dev벤치≠패리티` trap). S5 MUST add an explicit assertion for each before TCP goes live:
     1. **[from S2c-1] Lane assignment under a REAL split.** Entries-AE→`CallBulk` routing is inspection-verified only; on single-lane QUIC `CallBulk`==`Call` so no test distinguishes them, and a mis-wire would just HoL-block under load. Assert: with `SetMuxBulkLaneStreams(k>0)`, an entries-bearing AppendEntries is written on a bulk-lane stream and a concurrent heartbeat on a control-lane stream.
     2. **[from S2b-2] Capability-exchange parity.** QUIC's `GetOrConnectMux`/inbound run `doCapabilityExchangeDial`/`handleCapabilityExchange` (version/feature negotiation) before handing the conn to the mux handler; the TCP carrier carries version only in the ALPN (`tcpMuxALPN`). Confirm no OTHER negotiated capability is load-bearing — if one is, add a TCP equivalent before flipping boot.
     3. **[from S4] Join RST-robustness under latency.** The S4 loopback tests prove the half-close SEMANTIC only; RST-robustness is correct by construction (leader drain) but unproven empirically. Assert: a real multi-node / latency-injected TCP join reads the FULL reply (no RST-truncation) — the strongest form runs the real `inviteJoinDial` consumer against `NewTCPJoinListener`.
