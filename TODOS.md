...
- [ ] **NBD volume per-block file open ceiling** — 2026-05-08 `perf/nbd-bottlenecks` worktree에서 `bench_nbd_profile.sh` (single-node, encrypted, Colima loopback, nbd-client `-b 4096`) baseline. 결과: seq-read-4K 180 MiB/s (양호), seq-write-4K 24.0 MiB/s, seq-write-64K 44.1 MiB/s, write IOPS variance 60x (526~32K) — bursty stall 패턴. CPU 67% utilization (Iceberg/NFS 대비 높음, 진짜 워크 중). pprof 분석: `volume.blockIOEngine.writeDeferred` cum 14.14s (35%) — line 431 `e.objects.WriteAt(...)` 13.81s, `volume.blockIOEngine.read` cum 10.98s (27%) — line 174 `e.objects.GetObject(...)` 10.23s. **근본 원인**: NBD가 매 4KB block I/O마다 (a) cache-miss read는 `GetObject` → file open + io.ReadFull + close, (b) write는 `DistributedBackend.WriteAt` → 매 호출 file open + pwrite + close. 이로 인해 6K+ ops/sec 환경에서 12K+ open/close syscall/sec. **시도 (별도)**: 128 KiB buffer pool 추가 (`nbdKernelPoolBufSize`) — `-b 4096` 환경에서 0.025% CPU만 hit (dead code), revert. **다음 경로 (이번 cycle 밖)**: (a) **internal-bucket fd cache** — DistributedBackend에 path→fd LRU cache 추가, NFS WriteAt + NBD WriteAt + read path 모두 영향. NFS/Iceberg에서 같은 패턴 surface됐고 가장 큰 win 가능성 있지만 eviction/leak/Truncate semantics 검토 필요. (b) `volume.blockIOEngine.read` cache-miss 경로를 `GetObject` 대신 `ReadAt` + cache-populate로 — 단, DistributedBackend.ReadAt 자체도 file open per call이라 net 효과는 fd cache 도입 후에 의미. (c) NBD client block size 확대 (`-b 65536` or `-b 131072`) — 16~32x 적은 IOPS, 128 KiB pool 효과 검증 가능. server-side change 없이 측정 가능. **재오픈 트리거**: (1) production NBD workload SLO 침범, (2) fd cache 설계가 안전하다고 판단되는 시점 (NBD/NFS/Iceberg PutObject 핫패스에 일괄 적용), (3) `-b 65536` 측정에서 128 KiB pool 효과가 명확하면 별도 작은 PR로 복귀 검토.
