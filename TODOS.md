...
- [ ] **Volume scrub — 다중 노드 repair E2E** — v0.0.43 의 e2e 는 단일 노드 (1-node cluster) 만 커버. RepairReplica 의 peer-pull 경로는 단위 테스트도 통합 테스트도 없음 (위 항목 `RepairReplica 단위 테스트` 와 함께). 후속에서 `tryStartStaticMRCluster` 기반 3-node E2E 추가 — 한 노드의 블록을 truncate, scrub trigger, peer 에서 healthy bytes 가 가져와져 local 복구되는지 확인. `--detach` 후 `volume scrub status` polling. **선행 조건 (2026-05-05 grill 결과):** 아래 "Cluster Replication Reliability" 섹션의 (1)~(3) 가 먼저 fix 되어야 e2e 가 의미를 가짐 — 현재 e2e 환경에서 N×replication 자체가 1 replica 만 만들고 있어 peer-pull 테스트 토양 자체가 없음.
...
## Cluster Replication Reliability (precondition for multi-node scrub E2E)

**Discovered 2026-05-05** during grill on multi-node repair E2E. 4-attempt probe never produced ≥2 replicas in a 3-node cluster harness. Root cause analysis surfaced 5 issues; all 5 are GrainFS-wide concerns, not scrub-local.

- [ ] **(1) Shard-stream replication EOF in cluster cold-start** — *정합성 위협, root cause 미규명* — `cluster.DistributedBackend.putObjectNxSpooled` (line 862-885) 의 peer 복제 loop 가 cluster 직후 첫 write 시 `b.shardSvc.WriteShardStream` 에서 EOF 로 실패: `"stream shard to <peer>: decode response from <peer>: read header: EOF"`. timing 무관 — 12s sleep 후 재시도해도 동일 EOF. raft 는 정상 (group instantiation + peer membership 모두 완료된 상태에서 발생). QUIC transport 는 dial 성공하지만 `transport.CallWithBody` 의 응답 디코드에서 EOF — peer 의 WriteShard handler 가 응답 전 stream close 또는 body read 도중 EOF. **위험:** production 에서 cluster restart 직후 첫 write 들이 모두 단일 replica 로 끝나도 무경고 (debug-level skip 로그만). e2e 환경 재현 가능. **다음 단계:** `transport.CallWithBody` + WriteShard handler (`shard_service.go:163-185`) 의 stream lifecycle 추적, peer 측 handler 가 어느 시점에 close 하는지. real production 에서도 동일 현상인지 cluster 재시작 후 첫 write 의 disk replica 카운트로 확인 필요.

- [ ] **(2) PeerHealth one-strike-out — cold-start fragility** — *정합성 위협 (silent fallback)* — `internal/cluster/peer_health.go` 의 `MarkUnhealthy` 가 단 1회 EOF 로 호출되면 cooldown 10초 동안 `IsHealthy=false`. 그동안 `putObjectNxSpooled` 의 peer loop (line 867-870) 가 해당 peer 를 skip — debug-level 로그만 남김. cooldown 만료되면 `IsHealthy` 자동 true 복귀하지만 (1) 이 timing-independent 라 다음 시도도 EOF → 재차 unhealthy. **위험:** 정상 peer 가 단일 transient error 로 영구 skip 까진 아니지만 (cooldown 회복) 운영 시 silent skip 자체가 N×replication 보장을 약화시킴. **개선 방향:** (a) skip 시 warning 레벨 로그 + counter 노출, (b) MarkUnhealthy 임계값 (N consecutive failures) 도입, (c) cluster status admin endpoint 에 peer-health 노출.

- [ ] **(3) Silent N×replication skip — debug 레벨만** — *모니터링 gap* — `putObjectNxSpooled` line 868: `b.logger.Debug().Str("peer", peer).Msg("skipping unhealthy peer for replication")`. info 레벨 운영에서는 안 보임. 첫 실패만 warn ("data replication failed"), 그 후 모든 skip 이 silent. **개선:** (a) warning + counter (e.g. `grainfs_replication_skipped_total{peer,bucket}` Prometheus metric), (b) admin/health endpoint 에 "writes succeeded with <N replicas, expected M" 등 표면화, (c) 운영자 알림용 threshold (예: 3-node 클러스터에서 1 replica 로 fall back 시 critical).

- [ ] **(4) 기존 NBD multi-raft test 가 byte-level cross-node replica 미검증** — *test coverage gap* — `tests/e2e/multiraft_sharding_test.go:859` `TestE2E_MultiRaftSharding_NBDRoutesThroughCoordinator` 가 NBD write 후 다른 노드의 S3 ListObjects 로 검증 — 그러나 ListObjects 는 raft-replicated metadata 만 보면 됨. byte-level 데이터가 정말 multi-node 에 복제됐는지는 검증 안 함. v0.0.43.1 grill 에서 발견 — 이 테스트만 보면 N×replication 이 동작하는 것처럼 보이지만 disk 검증 시 1 replica 만 존재. **개선:** (a) 모든 노드의 dataDir 를 walk 해서 실제 file 존재 카운트 검증 추가, (b) 별도 byte-level replication coverage test 로 분리.

- [ ] **(5) Scrub Director wiring 가 `scrub-interval > 0` 에 묶임** — *admin trigger 의도치 않은 비활성화* — `cmd/grainfs/serve.go:1381` `if scrubInterval > 0` 안에 `scrubber.Director` 생성 + `adminDeps.Director = director` 까지 들어있음. 운영자가 periodic scrub 끄려고 `--scrub-interval=0` 으로 운영하면 admin 의 `volume scrub <name>` CLI 도 같이 죽음 — "scrub director not configured" 에러. periodic 과 admin trigger 는 직교 기능이라 분리 wiring 이 맞음. **수정:** Director 생성 + Register + Start 와 source registration 을 `if scrubInterval > 0` 밖으로 빼고, 안에는 `BackgroundScrubber.Start(ctx)` + ticker + placement monitor 만 남김. 영향: e2e cluster harness 가 `ScrubInterval="0"` 기본인데도 admin 테스트 가능. CLI 사용자 경험 일관성. follow-up cluster reliability 작업 전에 작은 단독 PR 로 가능.
