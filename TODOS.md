...
- [ ] **`cluster remove-peer` 진짜 liveness signal — metaRaft 노드 헬스 시그널** — `cluster-membership-cli` 라운드에서 `POST /api/cluster/remove-peer` + `cluster peers/events` CLI + joint-consensus 엔진 경로 + e2e (3-node, kill follower, happy-path remove-peer) 까지 ship. **현재 단순화:** `/api/cluster/status` 는 `peer_snapshot` 을 노출하고, remove-peer preflight 는 snapshot 의 membership-mutation policy 를 사용한다. 그래서 `self` 와 명시적 `live` 만 alive 로 세고 `configured` 는 unknown 으로 취급한다. 그러나 아직 metaRaft peer 의 active health/probe source 가 없으므로 대부분 remote voter 는 `configured` 상태에 머문다. dead-peer를 자동으로 **detect하지 않음** → 운영자가 외부 신호 (모니터링/SSH 접속 실패) 로 죽음을 확인한 뒤 명시 호출하거나 `force=true` 로 override 해야 한다. **후속 작업:** metaRaft 측에 별도 peerHealth/probe monitor를 두고 `PeerSnapshot()` 이 그 결과를 소스로 삼도록 교체. 이때 `cluster peers` STATE 컬럼이 진짜 down/alive 를 반영하고, pre-flight 가 "alive였는데 정족수 깨질" 케이스도 잡게 됨. **Re-open trigger:** 운영자가 자동 dead-peer detection 을 요구하거나 metaRaft peerHealth 설계를 시작할 때.
