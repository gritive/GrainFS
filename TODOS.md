...
- [ ] **Rolling upgrade safety** — *zero ops* — 버전 간 binary 교체로 downtime/데이터 손실 없음. **Design doc shipped**: `~/.gstack/projects/gritive-grains/whitekid-devel-design-20260514-143737-rolling-upgrade-safety.md` (v2, 2026-05-14 Codex review 반영). N→N+1 단방향 + Approach B (Compat Fabric sliced). Slice 1 → 4 → (2 ‖ 3) → 5 순서. **Slice 1 shipped v0.0.190.1**: `tests/compat/` CI lane (6 scenarios), `docs/COMPAT.md`, `make test-compat`. **Slice 4 snapshot-format header shipped v0.0.203.0**: `GFSNAP01` envelope, restore-time future-format rejection, live `TestHeadSnapshotReject`. **Tracking 항목**:
