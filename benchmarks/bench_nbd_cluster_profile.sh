...
NBD_EXPORT_NAME=$(bench_create_nbd_export_name "$BINARY" "$BENCH_DIR/n0/admin.sock" default)
...
bench_colima_ssh sudo nbd-client -d "$NBD_DEV" 2>/dev/null || true
bench_colima_ssh sudo nbd-client "$HOST_IP" "$LEADER_NBD_PORT" "$NBD_DEV" -b 4096 -N "$NBD_EXPORT_NAME"
