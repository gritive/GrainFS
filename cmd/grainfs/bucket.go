...
func init() {
	...
	bucketCmd.AddCommand(
		...
		// TODO(T8-T10): the upstream/policy/versioning subtrees are still
		// added below via the legacy builder-function pattern. Each subsequent
		// task converts one file to var pattern and removes its builder call
		// from here.
		bucketUpstreamCmd(),
		bucketPolicyCmd(),
		bucketVersioningCmd(),
