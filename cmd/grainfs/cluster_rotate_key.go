...
Workflow (online rolling, zero-downtime):
  ...
  4. The rotation worker propagates the new PSK to every peer's
     keys.d/current.key (peers read it from disk), so future restarts pick it
     up. Workers consume keys.d/next.key from disk during rotation.
