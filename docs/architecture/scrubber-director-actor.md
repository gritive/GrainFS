...
4. `Register`는 부팅 중 wiring 구간에서만 호출된다. `BlockSource`/`BlockVerifier`
   구현체가 다른 부팅 구간에서 생성되므로 `NewDirector` 한 번에
   주입하기는 불가 (별도 부팅 wiring 재구성 필요 — out of scope).
...
constructor 옵션으로 강제하는 대안은 부팅 wiring 재구성 동반 — 별도
candidate로 분리.
...
- **부팅 wiring 재구성으로 Register 제거**. constructor 옵션화는 부팅 구간
