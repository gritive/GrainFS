...
| Auth                    | Protocol credential SigV4 access   | Partial    | Server tests cover catalog-scoped Iceberg protocol credentials, read-only request handling, omitted-warehouse requests using the credential catalog, and protocol mismatch rejection. Real client smoke coverage uses DuckDB Iceberg REST Catalog `AUTHORIZATION_TYPE 'sigv4'` with a catalog-scoped protocol credential on single-node. Cluster real-client smoke remains pending. |
