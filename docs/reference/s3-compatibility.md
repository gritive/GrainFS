...
| Object basics     | CopyObject                              | Supported     | `x-amz-copy-source` (+ optional `?versionId`), `x-amz-metadata-directive` (COPY/REPLACE), `x-amz-tagging-directive`, and `x-amz-acl`. Full source GetObject authorization chain (pre-load IAM/bucket-policy + post-load ACL including per-version ACL when `?versionId` is specified); destination-bucket versioning is honored. Single-node and 4-node e2e cover SSE metadata preservation and cross-bucket copy with a tagging directive. (UploadPartCopy is documented separately below.) |
...
| Access control    | ACL header on object write/copy         | Partial       | Supported canned ACLs on write: `private`, `public-read`, `public-read-write`. Recognized-but-unsupported AWS canned values (`authenticated-read`, `bucket-owner-*`, `aws-exec-read`) and all `x-amz-grant-*` explicit ACL grant headers are rejected with `NotImplemented` (501) on PutObject, CopyObject, and CreateMultipartUpload — rather than silently downgraded to private. Single-node and 4-node cluster e2e cover the fail-closed rejection surface and the write-path allow-path (PutObject and CopyObject with supported values). ACL stamping on completed multipart objects is not yet implemented (ACL headers on `CreateMultipartUpload` pass through or are silently ignored). Anonymous read authorization for `public-read` objects (GET 200 without auth) requires a separate authz layer fix and is not yet covered. |
