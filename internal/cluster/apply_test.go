...
// applySoleAuth is a test helper: propose CmdSetBucketSoleAuthority{Bucket, State}.
func applySoleAuth(t *testing.T, f *FSM, bucket, state string) {
	t.Helper()
	require.NoError(t, applyCmdErr(t, f, CmdSetBucketSoleAuthority, SetBucketSoleAuthorityCmd{
		Bucket: bucket,
		State:  state,
	}))
}

// applySoleAuthWithFloor is a test helper: propose CmdSetBucketSoleAuthority with an
// EpochFloor value. Used to verify the monotonic floor behavior in snapshot restore.
func applySoleAuthWithFloor(t *testing.T, f *FSM, bucket, state string, floor uint32) {
	t.Helper()
	require.NoError(t, applyCmdErr(t, f, CmdSetBucketSoleAuthority, SetBucketSoleAuthorityCmd{
		Bucket:     bucket,
		State:      state,
...
// readEpoch reads the soleauth epoch for bucket from the FSM's store.
func readEpoch(t *testing.T, f *FSM, bucket string) uint32 {
	t.Helper()
	return fsmSoleAuthEpoch(t, f, bucket)
}

// TestApplySetBucketSoleAuthority_EpochFloorMonotonic verifies that EpochFloor
// raises the stored epoch but never lowers it, and that the floor works when
// the committed soleauth state is "off" (the pending→off abort case leaves
// epoch>0 but state=off, which is exactly the lossy scenario a restore must fix).
func TestApplySetBucketSoleAuthority_EpochFloorMonotonic(t *testing.T) {
	f := newTestFSMForSoleAuth(t)
	applyCmd(t, f, CmdCreateBucket, CreateBucketCmd{Bucket: "b"})

	applySoleAuth(t, f, "b", soleAuthPending) // off→pending: epoch 1
	applySoleAuth(t, f, "b", soleAuthOff)     // pending→off abort: epoch 2, state off

	// Floor=9 > current epoch (2): must raise epoch to 9.
	applySoleAuthWithFloor(t, f, "b", soleAuthOff, 9)
	require.Equal(t, uint32(9), readEpoch(t, f, "b"))

	// Floor=3 < current epoch (9): must NOT lower; epoch stays 9.
	applySoleAuthWithFloor(t, f, "b", soleAuthOff, 3)
	require.Equal(t, uint32(9), readEpoch(t, f, "b"))
}
