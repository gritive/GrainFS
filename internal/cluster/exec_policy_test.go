...
func TestLocalExecution_ResolveRead_StaleLeaderHintSignalsForward(t *testing.T) {
	gb := newGroupBackendWithRaftForTest(&flippableRaftNode{leaderSeq: []bool{false}})
	groups := &fakeLocalGroups{backends: map[string]*GroupBackend{"g1": gb}}
	e := NewLocalExecution(groups)
	got, err := e.ResolveRead(context.Background(), RouteTarget{GroupID: "g1", SelfIsLeader: true, SelfIsVoter: true})
	require.NoError(t, err)
	require.Nil(t, got)
}

func TestLocalExecution_ResolveRead_FollowerVoterSignalsForward(t *testing.T) {
	gb := newGroupBackendWithRaftForTest(&flippableRaftNode{leaderSeq: []bool{false}})
	groups := &fakeLocalGroups{backends: map[string]*GroupBackend{"g1": gb}}
	e := NewLocalExecution(groups)
	got, err := e.ResolveRead(context.Background(), RouteTarget{GroupID: "g1", SelfIsVoter: true})
	require.NoError(t, err)
	require.Nil(t, got)
}
