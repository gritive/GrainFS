...
type AppendObjectCmd struct {
	...
	SegmentETag      string
	PlacementGroupID string
	// VersionID is generated by the propose-time leader (UUIDv7) and frozen
	// into the Raft log so all replicas write the same versioned key. When
	// empty (legacy replay / direct apply-test fixtures), apply falls back to
	// the legacy ObjectMetaKey-only single-write path.
	VersionID string
}
