...
func TestRestore_DenylistSurvivesSnapshot(t *testing.T) {
	...
	s := spki(44)
	require.NoError(t, src.Peers().registerMember("node-revoked", s, "127.0.0.1:7044", true))
	...
	require.ErrorIs(t, dst.Peers().registerMember("node-revoked", s, "127.0.0.1:7044", true), errSPKIDenylisted)
}
