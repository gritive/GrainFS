...
func TestPutTraceStageVocabularyIsStable(t *testing.T) {
	...
	require.Equal(t, "segment_write_prepare", string(PutTraceStageSegmentWritePrepare))
	require.Equal(t, "ec_split", string(PutTraceStageECSplit))
	...
	require.Equal(t, "promote_staged_node_batch", string(PutTraceStagePromoteStagedNodeBatch))
