...
func TestQuarantineCorruptShardLocalAtShardKey_VersionExists(t *testing.T) {
	...
	// The FSM record drives ShardTargetStillReferenced; the quorum-meta blob is
	// the authority QuarantineObject reads/writes (blob-resident quarantine).
	selfAddr := b.selfAddr
	...
	// QuarantineObject reads/writes the quorum-meta blob (the authority); seed a
	// per-version blob so the quarantine SET has a base to RMW.
