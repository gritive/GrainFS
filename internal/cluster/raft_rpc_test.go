...
//
// raftRPCTimeout drives the RPCs to the data-raft node (serveruntime/run.go
...
// safety: a pathologically low --raft-election-timeout (below raftRPCTimeout)
...
func TestRaftRPCTimeout_BelowElectionTimeout(t *testing.T) {
	require.Less(t, raftRPCTimeout, raft.DefaultElectionTimeout,
		...
		raftRPCTimeout, raft.DefaultElectionTimeout)
}
