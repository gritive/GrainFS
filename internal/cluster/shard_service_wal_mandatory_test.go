...
func TestAppendShardDataWAL_ReplayRequiresFsync(t *testing.T) {
	dir := t.TempDir()
	keeper, clusterID := testDEKKeeper(t)
	svc := NewShardService(dir, transport.MustNewQUICTransport("test-cluster-psk"), WithShardDEKKeeper(keeper, clusterID), WithDataWAL(mustTestDataWALDEK(t, dir, keeper, clusterID)))
