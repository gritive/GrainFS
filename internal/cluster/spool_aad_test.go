...
func TestSpoolRecordAADFieldsDistinct(t *testing.T) {
	...
	a := encrypt.BuildAAD(encrypt.DomainSpool, clusterID[:], spoolRecordAADFieldsInto(nil, "cluster-spool:1", 0)...)
	b := encrypt.BuildAAD(encrypt.DomainSpool, clusterID[:], spoolRecordAADFieldsInto(nil, "cluster-spool:2", 0)...)
	...
	r := encrypt.BuildAAD(encrypt.DomainSpool, clusterID[:], spoolRecordAADFieldsInto(nil, "cluster-spool:1", 1)...)
	...
	s := encrypt.BuildAAD(encrypt.DomainShard, clusterID[:], spoolRecordAADFieldsInto(nil, "cluster-spool:1", 0)...)
