...
// testBalancerProposerForWarmup creates a BalancerProposer with stale detection via gossip.NodeStats.UpdatedAt.
func testBalancerProposerForWarmup(store *gossip.NodeStatsStore, peers []string, timeout time.Duration) *BalancerProposer {
...
func TestWarmupComplete_AllPeersRecent(t *testing.T) {
	...
	for _, id := range append(peers, "self") {
		store.Set(gossip.NodeStats{NodeID: id, DiskUsedPct: 10})
	}
...
func TestWarmupComplete_StaleAllPeers(t *testing.T) {
	...
	store := gossip.NewNodeStatsStore(ttl)
	peers := []string{"n1", "n2"}

	// Seed self
	store.Set(gossip.NodeStats{NodeID: "self", DiskUsedPct: 10})
	// Seed n1 as stale (will expire before warmup check)
	store.Set(gossip.NodeStats{NodeID: "n1", DiskUsedPct: 10})
	// n2 never gossiped

	p := testBalancerProposerForWarmup(store, peers, 30*time.Second)
	require.True(t, store.BackdateUpdatedAt("n1", ttl+20*time.Millisecond))
