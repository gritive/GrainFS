...
func TestRunCreatePostsCredentialAndPrintsSecret(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte(`{
			...
			"mode":"rw",
			"secret":"pcsec_once",
			"connection_hint":{"mount_path":"vol1/pc_123:pcsec_once"}
		}`))
	}))
	t.Cleanup(srv.Close)
	...
	require.Equal(t, CreateReq{SAID: "sa_app", Protocol: "nfs", Resource: "volume/vol1", Mode: "rw"}, got)
	require.Contains(t, out.String(), "id:              pc_123")
	require.Contains(t, out.String(), "secret:          pcsec_once")
	require.Contains(t, out.String(), "connection_hint: mount_path=vol1/pc_123:pcsec_once")
}
...
func TestClientListSendsFilters(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		require.Equal(t, "nfs", r.URL.Query().Get("protocol"))
		require.Equal(t, "volume/vol1", r.URL.Query().Get("resource"))
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{"credentials":[{"id":"pc_123","sa_id":"sa_app","protocol":"nfs","resource":"volume/vol1","mode":"rw"}]}`))
	}))
	t.Cleanup(srv.Close)

	resp, err := NewClientForURL(srv.URL).ListFiltered(context.Background(), "sa_app", "nfs", "volume/vol1")
	require.NoError(t, err)
