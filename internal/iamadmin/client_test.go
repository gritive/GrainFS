...
func TestClient_GrantPut(t *testing.T) {
	...
	c := NewClientForURL(srv.URL)
	if err := c.GrantPut(context.Background(), "sa-x", "b", "Write"); err != nil {
		t.Fatal(err)
	}
...
func TestClient_GrantDelete(t *testing.T) {
	var got map[string]string
	var method string
	mux := http.NewServeMux()
	mux.HandleFunc("/v1/iam/grant", func(w http.ResponseWriter, r *http.Request) {
	...
	c := NewClientForURL(srv.URL)
	if err := c.GrantDelete(context.Background(), "sa-x", "b"); err != nil {
		t.Fatal(err)
	}
...
func TestClient_GrantList_QueryParams(t *testing.T) {
	var sawURL string
	mux := http.NewServeMux()
	mux.HandleFunc("/v1/iam/grant", func(w http.ResponseWriter, r *http.Request) {
		sawURL = r.URL.String()
	...
	c := NewClientForURL(srv.URL)
	if _, err := c.GrantListRaw(context.Background(), "sa-1", "b-1"); err != nil {
		t.Fatal(err)
	}
	if !strings.Contains(sawURL, "sa=sa-1") || !strings.Contains(sawURL, "bucket=b-1") {
		t.Errorf("url = %q", sawURL)
	}

	// Empty filters → no query string.
	sawURL = ""
	if _, err := c.GrantListRaw(context.Background(), "", ""); err != nil {
		t.Fatal(err)
	}
