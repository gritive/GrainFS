...
func TestServiceCreateGetListAndHints(t *testing.T) {
	...
	secret, err := svc.Create(CreateRequest{
		SAID: "node-a", Protocol: ProtocolS3, Resource: "bucket/devdisk", Mode: ModeRW,
	})
	require.NoError(t, err)
	require.NotEmpty(t, secret.ID)
	require.NotEmpty(t, secret.Secret)
	require.Equal(t, secret.ID, secret.ConnectionHint["access_key_id"])
	...
	require.Equal(t, ProtocolS3, item.Protocol)
	require.Equal(t, "bucket/devdisk", item.Resource)
	require.Equal(t, ModeRW, item.Mode)

	items := svc.List(ListFilter{SAID: "node-a", Protocol: ProtocolS3, Resource: "bucket/devdisk"})
...
func TestServiceListFiltersByResource(t *testing.T) {
	...
	_, err := svc.Create(CreateRequest{SAID: "sa-a", Protocol: ProtocolS3, Resource: "bucket/one", Mode: ModeRW})
	require.NoError(t, err)
	_, err = svc.Create(CreateRequest{SAID: "sa-a", Protocol: ProtocolS3, Resource: "bucket/two", Mode: ModeRW})
	require.NoError(t, err)

	got := svc.List(ListFilter{Protocol: ProtocolS3, Resource: "bucket/two"})
	require.Len(t, got, 1)
	require.Equal(t, "bucket/two", got[0].Resource)
}
...
func TestMaterializeCreateMatchesServiceCreateShape(t *testing.T) {
	...
	item, secret, err := MaterializeCreate(CreateRequest{
		SAID: "node-a", Protocol: ProtocolS3, Resource: "bucket/devdisk", Mode: ModeRW, ExpiresAt: &expires, CreatedBy: "admin",
	}, now)
	require.NoError(t, err)
...
func TestServiceRotateAndRevoke(t *testing.T) {
	...
	first, err := svc.Create(CreateRequest{SAID: "node-a", Protocol: ProtocolS3, Resource: "bucket/devdisk", Mode: ModeRO})
	require.NoError(t, err)
...
func TestServiceAuthenticate(t *testing.T) {
	...
	_, err = svc.Authenticate(AuthenticateRequest{Protocol: ProtocolS3, Resource: "bucket/devdisk", Mode: ModeRW, Secret: "wrong"})
	require.ErrorIs(t, err, ErrNotFound)
	_, err = svc.Authenticate(AuthenticateRequest{Protocol: ProtocolS3, Resource: "bucket/other", Mode: ModeRW, Secret: secret.Secret})
	require.ErrorIs(t, err, ErrInvalid)
	_, err = svc.Authenticate(AuthenticateRequest{Protocol: ProtocolS3, Resource: "bucket/devdisk", Mode: ModeRO, Secret: secret.Secret})
	require.ErrorIs(t, err, ErrInvalid)
}
...
func TestServiceAuthenticateRejectsExpiredAndRevoked(t *testing.T) {
	...
	expired, err := svc.Create(CreateRequest{
		SAID: "node-a", Protocol: ProtocolS3, Resource: "bucket/expired", Mode: ModeRW, ExpiresAt: &past,
	})
	require.NoError(t, err)
	active, err := svc.Create(CreateRequest{
		SAID: "node-a", Protocol: ProtocolS3, Resource: "bucket/revoked", Mode: ModeRW,
	})
	require.NoError(t, err)
	require.NoError(t, svc.Revoke(active.ID))

	_, err = svc.Authenticate(AuthenticateRequest{Protocol: ProtocolS3, Resource: "bucket/expired", Mode: ModeRW, Secret: expired.Secret})
	require.ErrorIs(t, err, ErrExpired)
	_, err = svc.Authenticate(AuthenticateRequest{Protocol: ProtocolS3, Resource: "bucket/revoked", Mode: ModeRW, Secret: active.Secret})
...
func TestMaterializeRotateDoesNotMutateInput(t *testing.T) {
	...
	item, _, err := MaterializeCreate(CreateRequest{
		SAID: "node-a", Protocol: ProtocolS3, Resource: "bucket/devdisk", Mode: ModeRO,
	}, now)
	require.NoError(t, err)
...
func TestServiceValidationAndExpiry(t *testing.T) {
	...
	_, err := svc.Create(CreateRequest{SAID: "", Protocol: ProtocolS3, Resource: "bucket/devdisk", Mode: ModeRW})
	require.Error(t, err)
	_, err = svc.Create(CreateRequest{SAID: "node-a", Protocol: "ftp", Resource: "bucket/devdisk", Mode: ModeRW})
	require.Error(t, err)
	_, err = svc.Create(CreateRequest{SAID: "node-a", Protocol: ProtocolS3, Resource: "bad", Mode: ModeRW})
	require.Error(t, err)
	past := now.Add(-time.Second)
	secret, err := svc.Create(CreateRequest{SAID: "node-a", Protocol: ProtocolS3, Resource: "bucket/devdisk", Mode: ModeRW, ExpiresAt: &past})
	require.NoError(t, err)
	item, err := svc.Get(secret.ID)
	require.NoError(t, err)
	require.NotNil(t, item.ExpiresAt)
