...
func (s *BackgroundScrubber) RegisterSource(name string, src BlockSource, ver BlockVerifier) {
	if s.sources == nil {
		...
		s.verifiers = map[string]BlockVerifier{}
	}
	s.sources[name] = src
...
func (s *BackgroundScrubber) SourceRunOnce(ctx context.Context, scope ScrubScope) {
	for name, src := range s.sources {
		...
		for blk := range ch {
			select {
			case <-ctx.Done():
				for range ch { //nolint:revive // drain producer on cancel
				}
				return
			default:
			}
			...
			if vErr != nil {
				log.Warn().Str("source", name).Str("key", blk.Key).Err(vErr).Msg("scrub: verify failed")
				continue
			}
			if st.IsHealthy() {
				continue
			...
			if rerr := ver.Repair(ctx, blk); rerr != nil {
				log.Error().Str("source", name).Str("key", blk.Key).Err(rerr).Msg("scrub: repair failed")
			}
		}
	}
}
...
// Emitted by Scrubbable.ScanObjectsGrouped for the lifecycle worker.
//
...
// Tag matches the lifecycle / storage Tag shape; kept in this package so
// scrubber consumers do not import storage. lifecycle imports both packages.
type Tag struct {
	Key   string
	Value string
}

// MultipartUpload is one in-progress multipart upload, surfaced for
// AbortIncompleteMultipartUpload evaluation.
