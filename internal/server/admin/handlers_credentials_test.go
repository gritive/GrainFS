...
func TestCredentialHandlersCreateListGetRotateRevoke(t *testing.T) {
	...
	created, err := admin.CreateCredential(context.Background(), d, admin.CredentialCreateReq{
		SAID: "node-a", Protocol: "s3", Resource: "bucket/devdisk", Mode: "rw",
	})
	require.NoError(t, err)
	require.NotEmpty(t, created.ID)
	require.NotEmpty(t, created.Secret)
	require.Equal(t, created.ID, created.ConnectionHint["access_key_id"])

	listed, err := admin.ListCredentials(context.Background(), d, admin.CredentialListReq{SAID: "node-a", Protocol: "s3"})
	require.NoError(t, err)
...
func TestCredentialHandlersRejectInvalidExpiresAt(t *testing.T) {
	...
	_, err := admin.CreateCredential(context.Background(), d, admin.CredentialCreateReq{
		SAID: "node-a", Protocol: "s3", Resource: "bucket/devdisk", Mode: "rw", ExpiresAt: "tomorrow",
	})
	require.Error(t, err)
	var ae *admin.Error
	require.ErrorAs(t, err, &ae)
...
func TestCredentialHandlersAuthorizeCreateRotateRevoke(t *testing.T) {
	...
	created, err := admin.CreateCredential(context.Background(), d, admin.CredentialCreateReq{
		SAID: "sa-app", Protocol: "s3", Resource: "bucket/devdisk", Mode: "rw",
	})
	require.NoError(t, err)
...
func TestCredentialHandlersAuthorizeGetAndList(t *testing.T) {
	...
	_, err = admin.ListCredentials(context.Background(), d, admin.CredentialListReq{Protocol: "s3", Resource: "bucket/devdisk"})
	require.NoError(t, err)
	require.Equal(t, credentialAuthCall{
		principal: principal.ServiceAccount("sa-app"),
...
func TestCredentialHandlersAuthorizeOIDCActorInsteadOfTargetSA(t *testing.T) {
	...
	require.Equal(t, "protocol-credential/s3/bucket/devdisk", authz.calls[0].resource)
}
...
func TestCredentialHandlersValidateCreateBeforeAuthorize(t *testing.T) {
	...
	_, err := admin.CreateCredential(context.Background(), d, admin.CredentialCreateReq{
		SAID: "sa-app", Protocol: "bogus", Resource: "bucket/devdisk", Mode: "rw",
	})
	require.Error(t, err)
	var ae *admin.Error
	require.ErrorAs(t, err, &ae)
...
func TestCredentialHandlersDenyGetAndListBeforeRead(t *testing.T) {
	...
	listed, err := admin.ListCredentials(context.Background(), d, admin.CredentialListReq{Protocol: "s3", Resource: "bucket/devdisk"})
	requireCredentialForbidden(t, err)
	require.Empty(t, listed.Credentials)
}
...
func TestCredentialHandlersDenyOIDCListEvenWhenEmpty(t *testing.T) {
	...
	listed, err := admin.ListCredentials(ctx, d, admin.CredentialListReq{Protocol: "s3", Resource: "volume/missing"})
	...
	require.Equal(t, credentialAuthCall{
		...
		action:    "grainfs:CredentialList",
		resource:  "protocol-credential/s3/volume/missing",
	}, authz.calls[0])
}
...
func TestCredentialHandlersAuthorizeOIDCEmptyListWithPartialFilterResource(t *testing.T) {
	...
	listed, err := admin.ListCredentials(ctx, d, admin.CredentialListReq{Protocol: "s3"})

	require.NoError(t, err)
	require.Empty(t, listed.Credentials)
	require.Len(t, authz.calls, 1)
	require.Equal(t, "grainfs:CredentialList", authz.calls[0].action)
	require.Equal(t, "protocol-credential/s3/*", authz.calls[0].resource)
}
