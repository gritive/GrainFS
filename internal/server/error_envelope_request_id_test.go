...
//
// writeXMLError must embed the X-GrainFS-Request-Id value from the Hertz
// request context's K/V store (stashed there by WithRequestID) into the
// response body, so clients can correlate failures with server-side
// audit/log records without trusting only the response header.
package server
