...
func (state *bootState) instantiateGroupWithConfig(glc cluster.GroupLifecycleConfig, entry cluster.ShardGroupEntry) (*cluster.GroupBackend, error) {
	...
	if state.capabilityGate != nil {
		gb.SetCapabilityEvidenceSource(state.capabilityGate.EvidenceSnapshot)
	}
	return gb, nil
}
...
func bootOwnedGroupsAndEC(ctx context.Context, state *bootState) error {
	...
	if state.capabilityGate != nil {
		...
		// every placement node's gossiped commit-combined evidence before
		// collapsing promote+meta into one RPC round.
		distBackend.SetCapabilityEvidenceSource(state.capabilityGate.EvidenceSnapshot)
	}
