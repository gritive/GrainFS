...
func TestEnsureBulkCipherFormat(t *testing.T) {
	...
	t.Run("marker 5 with bulk data returns nil", func(t *testing.T) {
		dir := t.TempDir()
		require.NoError(t, os.WriteFile(filepath.Join(dir, "encryption.format"), []byte("5"), 0o600))
		err := EnsureBulkCipherFormat(dir, true)
		require.NoError(t, err)
	})
...
func TestEnsureBulkCipherFormatRejectsPreXAESDEK(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "2") || !strings.Contains(err.Error(), "5") {
		t.Fatalf("error should name the version mismatch (2 vs 5): %v", err)
	}
}
...
// loud-fail on this "5" binary rather than mis-read.
func TestEnsureBulkCipherFormatRejectsPriorVersion3(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "3") || !strings.Contains(err.Error(), "5") {
		t.Fatalf("error should name the version mismatch (3 vs 5): %v", err)
	}
}
...
// this "5" binary (R2 DEK-sealed IAM credentials). Pre-"5" IAM ciphertext
...
func TestEnsureBulkCipherFormatRejectsPriorVersion4(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "4") || !strings.Contains(err.Error(), "5") {
		t.Fatalf("error should name the version mismatch (4 vs 5): %v", err)
	}
}
...
func TestEnsureBulkCipherFormatStampsCurrentVersionOnFreshDir(t *testing.T) {
	...
	if strings.TrimSpace(string(got)) != "5" {
		t.Fatalf("fresh dir stamped %q, want \"5\"", got)
	}
}
