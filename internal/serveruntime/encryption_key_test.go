...
func TestEnsureBulkCipherFormat(t *testing.T) {
	...
	t.Run("marker 7 with bulk data returns nil", func(t *testing.T) {
		dir := t.TempDir()
		require.NoError(t, os.WriteFile(filepath.Join(dir, "encryption.format"), []byte("7"), 0o600))
		err := EnsureBulkCipherFormat(dir, true)
		require.NoError(t, err)
	})
	...
	t.Run("unknown marker version is refused", func(t *testing.T) {
		dir := t.TempDir()
		require.NoError(t, os.WriteFile(filepath.Join(dir, "encryption.format"), []byte("8"), 0o600))
		err := EnsureBulkCipherFormat(dir, true)
		require.Error(t, err)
...
func TestEnsureBulkCipherFormatRejectsPreXAESDEK(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "2") || !strings.Contains(err.Error(), "7") {
		t.Fatalf("error should name the version mismatch (2 vs 7): %v", err)
	}
}
...
// loud-fail on this "7" binary rather than mis-read.
func TestEnsureBulkCipherFormatRejectsPriorVersion3(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "3") || !strings.Contains(err.Error(), "7") {
		t.Fatalf("error should name the version mismatch (3 vs 7): %v", err)
	}
}
...
// this "7" binary (R2 DEK-sealed IAM credentials). Pre-"5" IAM ciphertext
...
func TestEnsureBulkCipherFormatRejectsPriorVersion4(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "4") || !strings.Contains(err.Error(), "7") {
		t.Fatalf("error should name the version mismatch (4 vs 7): %v", err)
	}
}
...
func TestEnsureBulkCipherFormatRejectsPriorVersion5(t *testing.T) {
	...
	err := EnsureBulkCipherFormat(dir, false)
	require.Error(t, err)
	require.Contains(t, err.Error(), "expected \"7\"")
}
...
func TestEnsureBulkCipherFormatStampsCurrentVersionOnFreshDir(t *testing.T) {
	...
	if strings.TrimSpace(string(got)) != "7" {
		t.Fatalf("fresh dir stamped %q, want \"7\"", got)
	}
}
