...
// transport selector after the S5c-3 flip: only "quic" opts out; "tcp" and an empty
// Transport (the default, incl. in-process callers) both select the TCP stack.
func TestOptionsToConfig_TransportSelection(t *testing.T) {
	...
	require.False(t, optionsToConfig(ServeOptions{Transport: "quic"}, ":0", nil, nil, nil).useTCPTransport,
		`Transport "quic" is the QUIC opt-out`)
	require.True(t, optionsToConfig(ServeOptions{Transport: ""}, ":0", nil, nil, nil).useTCPTransport,
		"empty Transport (default) must select TCP (matches the cobra flag default)")
}
