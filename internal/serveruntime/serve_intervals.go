...
// ValidateRequiredIntervals ensures scrub, EC reshard, and ring reshard intervals
// are never zero. These three are always-on: disabling them risks data integrity
// (scrub: silent corruption, ring reshard: stale placement, EC reshard: N× → EC backlog).
...
func ValidateRequiredIntervals(cfg *Config) {
	checks := []intervalCheck{
		...
		{"--ring-reshard-interval", &cfg.RingReshardInterval, time.Hour},
	}
