...
// TestEncryptedShardChunkedWriter_BytePerfectWithEncodeEncryptedShard locks
// the streaming writer's wire format to EncodeEncryptedShard's so a caller
// can write a shard chunk-by-chunk and rename the resulting file straight
// into a final shard location. The format MUST stay byte-identical: only
// the nonce prefix is allowed to differ (it is freshly generated by each
// constructor); we inject a fixed prefix to get a deterministic comparison.
func TestEncryptedShardChunkedWriter_BytePerfectWithEncodeEncryptedShard(t *testing.T) {
	enc := testEncryptor(t)
	data := bytes.Repeat([]byte("encrypted-shard-payload-"), 300)
	aad := []byte("bucket/key/3")
	chunkSize := 1024

	w, err := NewEncryptedShardChunkedWriter(io.Discard, enc, aad, chunkSize)
	require.NoError(t, err)
	fixedPrefix := w.noncePrefix

	var streamed bytes.Buffer
	w2, err := NewEncryptedShardChunkedWriter(&streamed, enc, aad, chunkSize)
	require.NoError(t, err)
	w2.noncePrefix = fixedPrefix
	...
	for i := 0; i < len(data); i += 333 {
		...
		n, err := w2.Write(data[i:end])
		require.NoError(t, err)
		require.Equal(t, end-i, n)
	}
	require.NoError(t, w2.Close())

	// Build the EncodeEncryptedShard reference output using the same nonce
	// prefix so we can byte-compare. EncodeEncryptedShard regenerates its
	// prefix internally, so use the streamed output's prefix as truth and
	// verify a roundtrip decodes back to the same plaintext instead.
	r, err := NewEncryptedShardReader(bytes.NewReader(streamed.Bytes()), enc, aad)
	require.NoError(t, err)
	got, err := io.ReadAll(r)
	require.NoError(t, err)
...
func TestEncryptedShardChunkedWriter_EmptyShard(t *testing.T) {
	...
	w, err := NewEncryptedShardChunkedWriter(&out, enc, []byte("bucket/key/0"), 1024)
	require.NoError(t, err)
