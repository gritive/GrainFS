...
var _ = ginkgo.Describe("Dynamic join quorum", func() {
	ginkgo.Context("Cluster3Node", func() {
		...
		// Spins up a 3-node ClusterModeDynamicJoin cluster, SIGKILLs the leader,
		// and asserts the two survivors elect a new leader. Before the
		// config.Peers self-exclusion fix the joined nodes carried a phantom
		// self-address so currentVoters had 4 strings -> quorum 3 -> 2 survivors
		// stuck. Regression for TODOS.md "DynamicJoin quorum inflation".
		ginkgo.It("re-elects a leader with two survivors", func() {
			t := ginkgo.GinkgoTB()
