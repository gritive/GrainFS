...
// TestECScrubTriggerE2E exercises the POST /v1/scrub admin endpoint. Both
// sub-tests share a 3-node static-peers cluster (single boot per group entry):
//
...
func TestECScrubTriggerE2E(t *testing.T) {
	...
	t.Run("Cluster3Node", func(t *testing.T) {
		...
		runECScrubTriggerCases(t, c)
	})
}

func runECScrubTriggerCases(t *testing.T, c *e2eCluster) {
	t.Helper()
	...
	t.Run("DedupHitReturnsExistingSession", func(t *testing.T) {
