...
// TestPhase0to2P9AnonClosesE2E exercises the NFS§B T12 / §9 T73 invariant
...
// the flip.
//
// The case is named NFS in the spec for naming parity; this e2e covers the 9P
// side. The NFSv4 wire-equivalent is deferred to tests/nfs4_colima/ where a
// real kernel mount can drive the protocol.
//
// SingleNode only — see F-§B-cluster-fixture-coupling. Single-node fixture
// starts in Phase 0 so we can drive the flip explicitly.
func TestPhase0to2P9AnonClosesE2E(t *testing.T) {
	t.Run("SingleNode", func(t *testing.T) {
		runPhase0to2P9Cases(t, newSingleNodeP9Target(t))
	})
}

func runPhase0to2P9Cases(t *testing.T, tgt *p9Target) {
	t.Helper()
	...
	// bucket is denied.
	t.Run("AnonMountActive_FlipPhase2_NextOpRejected", func(t *testing.T) {
		...
		require.Errorf(t, err,
			...
			tgt.name)
		require.Truef(t, isEACCES(err) || isENOENT(err),
